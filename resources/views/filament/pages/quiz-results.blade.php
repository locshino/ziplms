    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-6 text-white text-center">
                <div class="text-4xl font-bold mb-2">
                    {{ $this->attemptModel->score }}/{{ $this->quiz->max_points }}
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div class="space-y-6">
                ...
                <div class="border-l-4 rounded-lg p-6 transition-all duration-200
                    ...
                    <div class="flex justify-between items-start mb-4">
                        ...
                        <div class="flex items-center space-x-2">
                            @if($isCorrect)
                                <span class="bg-green-100 dark:bg-green-800 text-green-800 dark:text-green-200 text-xs font-medium px-2.5 py-0.5 rounded">
                                    Đúng
                                </span>
                            @elseif($isAnswered)
                                <span class="bg-red-100 dark:bg-red-800 text-red-800 dark:text-red-200 text-xs font-medium px-2.5 py-0.5 rounded">
                                    Sai
                                </span>
                            @else
                                <span class="bg-yellow-100 dark:bg-yellow-800 text-yellow-800 dark:text-yellow-200 text-xs font-medium px-2.5 py-0.5 rounded">
                                    Chưa trả lời
                                </span>
                            @endif
                    ...
                    <div class="mt-4 p-3 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded">
        ...
        <!-- Attempt History -->
        @if(count($this->attemptHistory) > 1)
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-4">Lịch sử làm bài</h3>
            <div class="overflow-x-auto">
                <table class="min-w-full table-auto">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-gray-700">
                            <th class="px-4 py-2 text-left text-gray-900 dark:text-white">Lần thử</th>
                            <th class="px-4 py-2 text-left text-gray-900 dark:text-white">Điểm</th>
                            <th class="px-4 py-2 text-left text-gray-900 dark:text-white">Phần trăm</th>
                            <th class="px-4 py-2 text-left text-gray-900 dark:text-white">Thời gian</th>
                            <th class="px-4 py-2 text-left text-gray-900 dark:text-white">Ngày làm</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($this->attemptHistory as $index => $historyAttempt)
                        <tr class="border-b dark:border-gray-600 {{ $historyAttempt['id'] == $this->attemptModel->id ? 'bg-blue-50 dark:bg-blue-900/30' : '' }}">
                            <td class="px-4 py-2 text-gray-900 dark:text-white">
                                {{ $index + 1 }}
                                @if($historyAttempt['id'] == $this->attemptModel->id)
                                    <span class="text-blue-600 dark:text-blue-400 font-medium">(Hiện tại)</span>
                                @endif
                            </td>
                            <td class="px-4 py-2 font-medium text-gray-900 dark:text-white">
                                {{ $historyAttempt['score'] }}/{{ $this->quiz->max_points }}
                            </td>
                            <td class="px-4 py-2 text-gray-900 dark:text-white">
                                {{ number_format($historyAttempt['percentage'], 1) }}%
                            </td>
                            <td class="px-4 py-2 text-gray-900 dark:text-white">
                                {{ $historyAttempt['time_spent'] ?? 'N/A' }}
                            </td>
                            <td class="px-4 py-2 text-gray-900 dark:text-white">
                                {{ $historyAttempt['completed_at'] ? \Carbon\Carbon::parse($historyAttempt['completed_at'])->format('d/m/Y H:i') : 'N/A' }}
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
