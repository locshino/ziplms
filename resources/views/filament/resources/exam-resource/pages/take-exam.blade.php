...
<x-filament-panels::page>
        ...
        <div class="p-6 text-center bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex items-center justify-center space-x-4">
                @if ($this->incompleteAttempt)
                    <x-filament::button wire:click="continueExam" color="warning">
                        Tiếp tục bài làm dở
                    </x-filament::button>
                @endif
    ...
    @else
                    ...
                    if (this.timeLeft > 0) {
                        this.timer = setInterval(() => {
                            ...
                            if (this.timeLeft <= 0) {
                                ...
                                @this.submitExam(); // Tự động nộp bài khi hết giờ
                            }
                        }, 1000);
                    }
        ...
        >
                ...
                <div class="p-6 bg-white rounded-lg shadow dark:bg-gray-800">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                            Câu {{ $currentQuestionIndex + 1 }}/{{ $questions->count() }}
                        </h3>
                        ...
                        @switch($questionType)
                            ...
                            @case(App\Enums\QuestionType::TrueFalse)
                                @foreach($currentQuestion->choices as $choice)
                                    <label class="flex items-center p-3 border rounded-lg cursor-pointer dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <input type="radio" 
                                               name="answer.{{ $currentQuestion->id }}" 
                                               wire:model.live="singleChoiceAnswers.{{ $currentQuestion->id }}"
                                               value="{{ $choice->id }}"
                                               class="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                        <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">{!! $choice->content !!}</span>
                                    </label>
                                @endforeach
                                @break
                            ...
                            @case(App\Enums\QuestionType::MultipleChoice)
                                @foreach($currentQuestion->choices as $choice)
                                    <label class="flex items-center p-3 border rounded-lg cursor-pointer dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <input type="checkbox" 
                                               wire:model.live="multipleChoiceAnswers.{{ $currentQuestion->id }}"
                                               value="{{ $choice->id }}"
                            ...
                            @case(App\Enums\QuestionType::ShortAnswer)
                                <input type="text"
                                    wire:model.live.debounce.500ms="shortAnswers.{{ $currentQuestion->id }}"
                                    placeholder="Nhập câu trả lời của bạn"
                                    class="block w-full transition duration-75 border-gray-300 rounded-lg shadow-sm fi-input focus:border-primary-500 focus:ring-1 focus:ring-inset focus:ring-primary-500 disabled:opacity-70 dark:bg-gray-700 dark:text-white dark:border-gray-600 dark:focus:border-primary-500"
                                >
                                @break

                            @case(App\Enums\QuestionType::Essay)
                                <textarea
                                    wire:model.live.debounce.500ms="essayAnswers.{{ $currentQuestion->id }}"
            ...
            {{-- Cột điều hướng và thông tin --}}
            <div class="lg:col-span-4">
                <div class="sticky p-6 bg-white rounded-lg shadow top-20 dark:bg-gray-800">
                    {{-- Thời gian --}}
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-5 gap-2 md:grid-cols-8 lg:grid-cols-5">
                            @foreach ($questions as $index => $question)
                                ...
                                <button
                                    ...
                                    class="flex items-center justify-center w-10 h-10 text-sm font-medium border rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 focus:ring-primary-500"
                                    :class="{
